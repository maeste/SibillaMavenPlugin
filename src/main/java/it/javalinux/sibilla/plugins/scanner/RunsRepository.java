...
/**
 * Keep track of when target source files have gone through a Sibilla run last time
 * 
...
public class RunsRepository {
	...
	try {
	    ...
	    log.log(Level.INFO, "Unable to deserialize Sibilla runs repository from file: " + file, e);
	    return new HashMap<String, Long>();
	} finally {
	    try {
		ois.close();
	...
	try {
	    ...
	    log.log(Level.INFO, "Unable to serialize Sibilla runs repository to file: " + file, e);
	    throw e;
	} finally {
	    try {
		oos.close();
